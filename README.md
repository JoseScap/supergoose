...
If both `SUPERGOOSE_ROOT_USERNAME` and `SUPERGOOSE_ROOT_PASSWORD` are provided, the API seeds that root user into `supergoose_control` on startup when that username does not already exist.
